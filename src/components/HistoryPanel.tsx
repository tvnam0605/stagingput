...
export function HistoryPanel() {
  ...
  function exportCSV() {
    const hdr = 'PalletCode,Prefix,DateStr,Status,BoxCount,Note,CreatedAt,DoneAt'
    const rows = pallets.map(p =>
      `${p.code},${p.prefix},${p.date_str},${p.status},${(p.boxes ?? []).length},"${p.note ?? ''}","${p.created_at}","${p.done_at ?? ''}"`
    )
    ...
    <div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f5f4f0' }}>
                {['Mã Pallet', 'Status', 'Box', 'Ghi chú', 'Tạo lúc', 'Done lúc', ''].map(h => (
            ...
            <tbody>
              {pallets.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: 32, color: '#a09e96', fontSize: 13 }}>Không tìm thấy</td></tr>
              ) : pallets.map(p => (
      ...
      {selected && (
        <div onClick={e => e.target === e.currentTarget && setSelected(null)}
          ...
          <div style={{ background: '#fff', borderRadius: 16, width: 480, maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 8px 40px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ padding: '20px 24px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
                {[
                  ...
                  { k: 'Chốt lúc', v: selected.done_at ? fmt(selected.done_at) : '—' },
                ].map(item => (
              ...
              {!(selected.boxes ?? []).length ? (
              ...
              ) : (selected.boxes ?? []).map((b: Box, i: number) => (
                <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#f5f4f0', borderRadius: 8, padding: '8px 12px', marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 11, color: '#a09e96', fontFamily: F.code }}>
